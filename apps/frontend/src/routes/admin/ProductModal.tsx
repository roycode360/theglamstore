...
export default function ProductModal({
  ...
  const { data: fullData } = useQuery(GET_PRODUCT, {
    variables: { id: initial?.id },
    skip: !(open && isEdit && initial?.id),
  ...
  const categoryOptions = (catsData?.listCategories ?? [])
    .filter((c: any) => c.active)
    .map((c: any) => ({ value: c.slug, label: c.name }));
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        await update({ variables: { id: initial.id, input } });
            ...
            <label className="block text-sm">
              ...
              <div className="mt-1 flex gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    setSku(
                      generateBaseSKU(category, name, { maxSegmentLength: 8 }),
                    )
                  }
                  className="theme-border shrink-0 rounded-md border bg-white px-3 py-2 text-sm hover:bg-gray-50"
                >
