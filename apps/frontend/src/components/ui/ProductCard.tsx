...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const defaultColor = rawColor
    ...
    : '';
  const primaryImage =
    getOptimizedImageUrl(product.images?.[0]) ?? product.images?.[0];
  const secondaryImage =
    getOptimizedImageUrl(product.images?.[1] || product.images?.[0]) ??
    product.images?.[1] ??
    product.images?.[0];
    ...
    >
      ...
      >
        ...
        <div className="relative aspect-[3/4] w-full overflow-hidden bg-white">
          ...
          {isOutOfStock && (
            <span className="absolute bottom-3 left-3 z-10 inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-gray-600">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-3 w-3"
                viewBox="0 0 24 24"
          ...
          {isHovered && (
            <div className="absolute bottom-3 right-3 z-10 flex flex-col items-center gap-4">
              ...
              <button
                ...
                className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-black transition-all duration-1000 ease-out hover:bg-black hover:text-white hover:shadow-lg disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400 disabled:hover:bg-gray-200 disabled:hover:text-gray-400"
